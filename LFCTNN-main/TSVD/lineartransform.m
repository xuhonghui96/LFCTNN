% function A = lineartransform(A,transform)

% perform linear transform on tensor A along 3rd dim
%
%   A           - n1*n2*n3 tensor
...
%
%	Examples: 
%            n = 10; A = rand(n,n,n); A = lineartransform(A,transform);
...
%                      A = lineartransform(A,transform) is equivalent to A = fft(A,[],3);
%             (2) dct: transform.L = @dct, transform.l = 1, transform.inverseL = @idct. 
%                      A = lineartransform(A,transform) is equivalent to A = dct(A,[],3);
...
%
%
% See also inverselineartransform
%
%
% References:
...
function A = lineartransform(A,transform)
if nargin < 2
    A = fft(A,[],3); % fft is the default transform
